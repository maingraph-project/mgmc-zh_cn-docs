...
- 例外：客户端节点文档存放在 `zhcn-docs/docs/client_nodes/{category_path}/{id}.md`。
- **分类路径**：`category_path` 应根据 i18n 中的 `node_category.mgmc.*`（node_category.mgmc_client.*） 层级确定，点号替换为斜杠（例如 `variable/string`）。
