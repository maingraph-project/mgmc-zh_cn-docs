...
| **持续时间 (秒)** (Duration) | 浮点数 (Float) | 效果持续的时间（秒）。默认为 `10.0`。 |
| **效果等级** (Amplifier) | 浮点数 (Float) | 效果的等级（从 0 开始计数，0 表示 I 级，1 表示 II 级）。默认为 `0.0`。 |
...
3. **时间转换**：输入的秒数会被转换为游戏刻（Ticks，1秒 = 20刻）。
4. **有效性检查**：效果名称必须是有效的 Minecraft 药水效果 ID。如果 ID 错误，节点将不会产生任何效果。
