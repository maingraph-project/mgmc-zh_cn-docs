...
我们要实现一个简单的逻辑：**当输入指定的命令，在聊天栏显示“Hello, World!”**

### 第一步：添加“事件”节点
1. 在编辑器的黑色画布空白处，点击 **鼠标右键**。
2. 选择 **事件** -> [当运行 /mgrun命令时](/节点/事件/玩家事件/当运行mgrun命令时)。
3. 此时画布上会出现一个节点，它是逻辑的起点。

![一张GIF，展示了添加事件节点的过程](https://t.tutu.to/img/MmHHH)

### 第二步：添加“动作”节点
1. 再次在空白处点击 **鼠标右键**。
2. 选择 **动作 (Actions)** -> **玩家 (Player)** -> [打印到聊天栏](/节点/动作/玩家动作/打印到聊天栏) (Print Chat)。
...
### 第四步：设置消息内容
1. 在“[打印到聊天栏](/节点/动作/玩家动作/打印到聊天栏)”节点中，找到名为 `消息 (Message)` 的端口。
